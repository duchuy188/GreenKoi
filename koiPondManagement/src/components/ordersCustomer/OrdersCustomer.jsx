...
const OrdersCustomer = () => {
  ...
  return (
    <div className="container py-4 bg-light">
      ...
      <Modal
        title={<h4 className="text-warning mb-0"><i className="fas fa-info-circle me-2"></i>Chi Tiết Đơn Hàng</h4>}
          ...
          <div className="p-3">
            <div className="row">
              <div className="col-md-6">
                <p><i className="fas fa-tag text-warning me-2"></i><strong>Tên:</strong> {selectedOrder.name || 'N/A'}</p>
                <p><i className="fas fa-align-left text-orange me-2"></i><strong>Mô Tả:</strong> {selectedOrder.description || 'N/A'}</p>
                <p><i className="fas fa-dollar-sign text-warning me-2"></i><strong>Tổng Giá:</strong> {selectedOrder.totalPrice != null ? `$${Number(selectedOrder.totalPrice).toFixed(2)}` : 'N/A'}</p>
                <p><i className="fas fa-piggy-bank text-orange me-2"></i><strong>Số Tiền Đặt Cọc:</strong> {selectedOrder.depositAmount != null ? `$${Number(selectedOrder.depositAmount).toFixed(2)}` : 'N/A'}</p>
              </div>
              <div className="col-md-6">
                <p><i className="fas fa-chart-line text-warning me-2"></i><strong>Trạng Thái:</strong> <span className="badge bg-warning text-dark">{selectedOrder.statusName || 'N/A'}</span></p>
                <p><i className="fas fa-calendar-alt text-orange me-2"></i><strong>Ngày Bắt Đầu:</strong> {selectedOrder.startDate ? moment(selectedOrder.startDate).format('DD/MM/YYYY') : 'N/A'}</p>
                <p><i className="fas fa-calendar-check text-warning me-2"></i><strong>Ngày Kết Thúc:</strong> {selectedOrder.endDate ? moment(selectedOrder.endDate).format('DD/MM/YYYY') : 'N/A'}</p>
                <p><i className="fas fa-user-tie text-orange me-2"></i><strong>Mã Tư Vấn Viên:</strong> {selectedOrder.consultantId || 'N/A'}</p>
              </div>
            </div>
            <h5 className="mt-4 mb-3 text-warning"><i className="fas fa-tasks me-2"></i>Công Việc:</h5>
            <div className="table-responsive">
              <table className="table table-striped table-hover">
                <thead className="table-warning">
                  <tr>
                    <th><i className="fas fa-clipboard-list text-warning me-2"></i>Tên Công Việc</th>
                    <th><i className="fas fa-info-circle text-orange me-2"></i>Trạng Thái</th>
                    <th><i className="fas fa-percentage text-warning me-2"></i>Hoàn Thành</th>
                  </tr>
                </thead>
                <tbody>
                  {(selectedOrder.tasks || []).map((task, index) => (
                    <tr key={index}>
                      <td>{task.name}</td>
                      <td>{task.status}</td>
                      <td>{task.completionPercentage != null ? `${task.completionPercentage}%` : 'N/A'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <Modal
        title={<h4 className="text-warning mb-0"><i className="fas fa-star me-2"></i>Gửi Đánh Giá</h4>}
      ...
      >
        <Form form={form} onFinish={submitReview} layout="vertical">
          <Form.Item name="rating" label={<span><i className="fas fa-star text-warning me-2"></i>Đánh Giá</span>} rules={[{ required: true, message: 'Vui lòng đánh giá dự án' }]}>
            <Rate />
          </Form.Item>
          <Form.Item name="comment" label={<span><i className="fas fa-comment text-orange me-2"></i>Bình Luận</span>} rules={[{ required: true, message: 'Vui lòng để lại bình luận' }]}>
            <Input.TextArea rows={4} className="form-control" />
          </Form.Item>
          <Form.Item>
            <Button type="primary" htmlType="submit" className="btn btn-warning text-dark w-100">
